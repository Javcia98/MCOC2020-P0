...
3. El tamaño de las matrices afecta al comportamiento en el sentido de que a mas grande la matriz mas tiempo se demora en correr en programa, esto ocurrio con todos los casos. Llegando a comportarse como una funcion exponencial (lineas de colores), en todos los graficos se presentaron tendencias: En el de ensamblado normalmente la pendiente promedio de las corridas era igual a la de O(N^2) aproximadamente. En el grafico de solucion pasasba lo mismo pero con la O(N^3) y en algunos casos extremos a O (N^4). <br>
4. La estabilidad de las corridas dependera mucho del computador de y las otras tareas que este realizando. Mientras mas constante sean las tareas que debe ahcer el cumputador mas estables seras las corridas, pero cuando no se puede ver que son muy distitnas unas de las otras con unos saltos muy notorios. Personalmente me pude dar cuenta de este problema al hacer esta entrega ya que al mismo tiempo tuve que estar con el Autocad haciendo unos trabajos y obviamente esucchando musica por Spotify. El uso de estas aplicaciones (abrir, cerrar, crear otros archivos y cambiar de cancion) hizo que se perdiera estabilidad en algunos momentos en que se corria el codigo, ergo las diferencias de las curvas grises en muchos casos. Pero en una situacion donde solamente este enfocado en el codigo y lo haga correr entonces la estabilidad debiera ser mucho mayor.  <br>
